...
kenties olisi valmis ottamaan paikan vastaan. Syntyy erilaisten paikan
vahvistus käytäntöjen ja jonotuslistojen hankaloittama mekanismi.
...
oppilaitosten prioriteeteissä. Prioriteettien vaihtamista voidaan
havainnollistaa esimerkillä, jossa $O = \{o_1, o_2\}$ ja $H = \{h_1,
h_2\}$ ja toimijoiden mieltymykset taulukon \ref{top_esimerkki}
mukaiset.
...
erikoistapauksen ajassa $O(\sqrt(n)m)$, missä $n$ on kaikkien
toimijoiden määrä ja $m$ kaikkien preferenssirelaatioiden yhteen
laskettu pituus listana kuvattuna \cite{abraham05}.
Opiskelijavalintaongelman tilanne voidaan kuitenkin kuvata edellä
kuvatun erikoistapauksen avulla. Jokainen oppilaitos monistetaan sen
avoimien paikkojen määrään verran ja jokaiselle kopiolle asetetaan
yksi avoin paikka. Lisäksi jokainen monistettuun oppilaitokseen
hakenut hakija asetetaan hakemaan kaikkiin oppilaitoksen kopioihin.
Hakijan mieltymyksissä monistettu oppilaitos korvataan jollain,
kaikille oppilaitokseen hakeneille samalla, järjestyksellä kopioista.
